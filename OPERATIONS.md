...
> **`0.0.0.0` 은 접속 주소가 아니다.** "모든 인터페이스에서 듣는다"는 뜻이고,
> 앱 설정에 그대로 넣으면 접속되지 않는다. 허브가 시작할 때 실제로 넣어야 할 주소를 찍어 준다.
>
> ```
> [hub] listening on ws://0.0.0.0:8787/ws
> [hub] 앱 설정에 넣을 주소 (0.0.0.0 을 그대로 넣으면 접속되지 않습니다):
> [hub]   192.168.0.5:8787  (wlan0)
> [hub]   100.101.102.103:8787  (tun0)  ← Tailscale, 집 밖에서도 됨
> ```
>
> 나중에 다시 확인하려면 `head -20 ~/hub.log` 를 보거나 허브를 재시작한다.
